// Web Audio API micro-sound synthesizers for interactive feedback
class SoundController {
  ...
  // Biometric laser scan pulse sound
  playScanPulse() {
    if (!this.soundEnabled) return;
    const ctx = this.getContext();
    if (!ctx) return;
  ...
  // Positive verification success sound
  playSuccess() {
    if (!this.soundEnabled) return;
    const ctx = this.getContext();
    if (!ctx) return;
  ...
  // Negative or warning error buzzer sound
  playError() {
    if (!this.soundEnabled) return;
    const ctx = this.getContext();
    if (!ctx) return;
  ...
  // Light / Dark mode toggle switch sound
  playToggle() {
    if (!this.soundEnabled) return;
    const ctx = this.getContext();
    if (!ctx) return;
