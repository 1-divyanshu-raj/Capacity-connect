...
import { 
  MOCK_COURSES, MOCK_ASSESSMENTS, MOCK_LIBRARY_RESOURCES, MOCK_PENDING_APPROVALS,
  MOCK_ANNOUNCEMENTS, MOCK_COMPETENCY_MATCHES, MOCK_TRAINEE_PROGRESS
} from './data/mockData';
...
export default function App() {
  ...
  if (authLoading && !currentUser) {
    return (
      <div className="min-h-screen grid place-items-center bg-[#f8fafc] dark:bg-[#090e17] text-slate-600 dark:text-slate-300">
        <div className="text-center"><div className="animate-spin h-8 w-8 rounded-full border-2 border-rose-500 border-t-transparent mx-auto mb-3" /><p>Restoring secure session…</p></div>
      </div>
    );
  }
    ...
    <div className="min-h-screen flex flex-col bg-[#f8fafc] dark:bg-[#090e17] text-slate-900 dark:text-slate-100 selection:bg-rose-400 selection:text-white relative overflow-x-hidden transition-colors duration-200">
      ...
      <footer className="relative z-10 py-3.5 border-t border-slate-200/90 dark:border-slate-800 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md text-[11px] text-slate-500 dark:text-slate-400 mt-auto transition-colors duration-200">
        <div className="max-w-7xl mx-auto px-4 lg:px-8 flex flex-col sm:flex-row items-center justify-between gap-3">
          <div className="flex items-center gap-2"><span className="font-semibold text-slate-700 dark:text-slate-200">CAPACITY CONNECT</span><span>•</span><span>Ministry of Earth Sciences | IMD - Training Division</span><span>•</span><span className="font-mono text-rose-700 dark:text-rose-400 font-medium">Govt. of India</span></div>
          <div className="flex items-center gap-4 text-slate-400 dark:text-slate-500"><span>Security Standards</span><span>•</span><span>Helpdesk: 1800-MOES-CONNECT</span><span>•</span><span className="font-mono text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded border border-slate-200/70 dark:border-slate-700">ROLE: {currentUser.role.toUpperCase()}</span></div>
        </div>
      </footer>
    </div>
  );
